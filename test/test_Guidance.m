...
time = 60*60*5;%(s) 2*pi*sqrt(a^3/earth.Mu)
dt = 10;%(s)
...
% 数值求解轨道, 姿态
[r_list, v_list, e_list, w_list] = simulation(r0, v0, e0, w0, W0, time, dt);
...
for n = 1:length - 1
    ...
    DrawCube(8);
    satellite.DrawCubeSat(time, 2, 1);
    satellite.DrawOrbit(time,2)
    earth.DrawEarth(time,2);
    sun.DrawSun(time,0,4);
    sun.DrawShad(time,2,4);
    sun.DrawSunLight(time, 3, 0.5);

    disp(satellite.StatusList(i, 1));
